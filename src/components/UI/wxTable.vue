<template>
  <div>
    ...
    <DataTable :value="items" showGridlines :loading="busy" stripedRows>
      <template #Loading>
      ...
      <Column v-for="col of columns" :key="col.field" :field="col.field" :header="col.header">
        <template #body="{ data, field }">
          ...
          <template v-else-if="field === 'txn_amount'">
            <span :class="[data.txn_type == 'CREDIT' ? 'text-green-600' : 'text-red-600' ]">
              {{  `${data.txn_type == 'CREDIT' ? '+' : '-'}${$currencyFormat(data.amount)}` }}
            </span>
          </template>
...
.p-datatable .p-datatable-thead > tr > th:first-child, .p-datatable .p-datatable-tbody > tr > td:first-child {
  /* border-radius: 30px !important;  */
  border-left: 1px solid #E0E1E6 !important;
}

.p-datatable .p-datatable-thead > tr > th:last-child, .p-datatable .p-datatable-tbody > tr > td:last-child {
